...
/////////////////////////////////////////////////////////////
package com.alinesno.infra.data.pipeline.dialect;
  ...
  private int parseLine(BufferedReader reader, int lc, List<String> names)
    ...
    if (n != 0) {
      ...
      if (!Character.isJavaIdentifierStart(cp)) {
        log.error("Illegal isJavaIdentifierStart provider-class name: {}", ln);
      }
      for (int i = Character.charCount(cp); i < n; i += Character.charCount(cp)) {
        cp = ln.codePointAt(i);
        if (!Character.isJavaIdentifierPart(cp) && (cp != '.')) {
          log.error("Illegal isJavaIdentifierPart provider-class name: {}", ln);
        }
      }
