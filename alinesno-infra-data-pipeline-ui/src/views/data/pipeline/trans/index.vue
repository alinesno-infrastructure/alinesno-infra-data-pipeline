        ...
        <el-col :span="24" :xs="24">
           ...
           <el-table v-loading="loading" :data="TransList" @selection-change="handleSelectionChange">
              ...
              <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                 <template #default="scope">
                    <el-tooltip content="周期" placement="top" v-if="scope.row.TransId !== 1">
                       <el-button link type="primary" icon="PieChart" @click="handleUpdate(scope.row)" v-hasPermi="['system:Trans:edit']"></el-button>
                    </el-tooltip>
                    <el-tooltip content="修改" placement="top" v-if="scope.row.TransId !== 1">
                       <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:Trans:edit']"></el-button>
                    </el-tooltip>
                    <el-tooltip content="删除" placement="top" v-if="scope.row.TransId !== 1">
                       <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:Trans:remove']"></el-button>
                    </el-tooltip>
                 </template>
...
function getList() {
  loading.value = true;
  listTrans(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
     loading.value = false;
...
function handleDelete(row) {
  const TransIds = row.id || ids.value;
  proxy.$modal.confirm('是否确认删除转换编号为"' + TransIds + '"的数据项？').then(function () {
     return delTrans(TransIds);
  }).then(() => {
     getList();
...
function handleUpdate(row) {
  reset();
  const TransId = row.id || ids.value;
  getTrans(TransId).then(response => {
...
function submitForm() {
  proxy.$refs["databaseRef"].validate(valid => {
     if (valid) {
        if (form.value.TransId != undefined) {
